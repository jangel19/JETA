  ...
  return (
    <div className="mx-auto max-w-screen-xl px-4 md:px-8 py-10 md:py-14">
      <div className="flex items-center gap-2">
        <button className={`btn ${tab==="flip"?"btn-primary":"btn-ghost"}`} onClick={() => setTab("flip")}>Flip</button>
        <button className={`btn ${tab==="rental"?"btn-primary":"btn-ghost"}`} onClick={() => setTab("rental")}>Rental</button>
      </div>
        ...
        {tab === "flip" ? (
          <form onSubmit={flipForm.handleSubmit(onSubmit)} className="grid md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 flex items-center gap-3 mt-2">
              <button disabled={loading} className="btn btn-primary">{loading ? "Analyzing…" : "Analyze deal"}</button>
              <button type="button" className="btn btn-ghost" onClick={() => flipForm.reset()}>Reset</button>
            </div>
          </form>
        ) : (
          <form onSubmit={rentalForm.handleSubmit(onSubmit)} className="grid md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 flex items-center gap-3 mt-2">
              <button disabled={loading} className="btn btn-primary">{loading ? "Analyzing…" : "Analyze deal"}</button>
              <button type="button" className="btn btn-ghost" onClick={() => rentalForm.reset()}>Reset</button>
            </div>
          </form>
        )}
      </div>

      {error && <div className="mt-4 p-4 rounded-xl border border-red-200 bg-red-50 text-red-700">{error}</div>}
      {result && <div className="mt-6"><ResultCard data={result} /></div>}
    </div>
  );
}
...
function Field({ f, name, label, type="text", step }: any) {
  ...
  return (
    <div>
      ...
      <input type={type} step={step} {...register(name, { valueAsNumber: ["number"].includes(type) })} className="input w-full mt-1" />
      {errors[name] && <div className="text-sm text-red-600 mt-1">{errors[name].message as string}</div>}
    </div>
  );
}
function Select({ f, name, label, options }: any) {
  ...
  return (
    <div>
      <label className="text-sm text-subtle">{label}</label>
      <select {...register(name)} className="input w-full mt-1">
        {options.map((o: string) => <option key={o} value={o}>{o[0].toUpperCase()+o.slice(1)}</option>)}
      </select>
      {errors[name] && <div className="text-sm text-red-600 mt-1">{errors[name].message as string}</div>}
    </div>
  );
  ...
  return (
    <div className="md:col-span-2">
      ...
      <input
        ...
        defaultValue={raw}
        onChange={(e) => {
          const arr = e.target.value.split(",").map(s => Number(String(s).trim())).filter(n => !Number.isNaN(n));
